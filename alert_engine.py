...
ZONE QUALITY (3pts): +1 Valid OB at zone | +1 FVG overlaps OB | +1 Zone fresh (tested 2 times or less)
...
If score < {min_confidence}: set send_alert to false.
If score >= {min_confidence}: set send_alert to true.

LEVEL RULES — use ONLY candle data above:
- SL: below OB wick for longs, above OB wick for shorts
- Entry: inside OB or FVG range
- TP1: nearest swing high or low in candle data
- TP2: next major structure level in candle data
- Never guess. If candle data does not support a level, say so.
...
{{
  ...
  "news_flag": "none or describe upcoming event",
  ...
  "lot_size": "x.x",
  "sl_pts": 0,
  "trigger": "exact M15 or H1 price action before entry",
  "invalid_if": "exact condition that kills the trade",
...
def generate_chart(df, title, levels, data):
    try:
        if df is None or df.empty:
            print(f"    Chart skipped: empty dataframe for {title}")
            return None
        ...
        for col in ['Open','High','Low','Close']:
            if col not in df_plot.columns:
                print(f"    Chart skipped: missing column {col}")
                return None
            df_plot[col] = pd.to_numeric(df_plot[col], errors='coerce')
        df_plot = df_plot.dropna(subset=['Open','High','Low','Close'])

        if len(df_plot) < 5:
            print(f"    Chart skipped: not enough candles")
            return None

        fig, (ax, ax_vol) = plt.subplots(
            2, 1, figsize=(13, 7), facecolor='#131722',
        ...
        # OB
        try:
            ob_top    = float(data.get('ob_top') or 0)
            ob_bottom = float(data.get('ob_bottom') or 0)
            if ob_top > 0 and ob_bottom > 0 and ob_top != ob_bottom:
                ...
                ax.add_patch(patches.Rectangle(
                    ...
                    facecolor=ob_color, alpha=0.15 if confirmed else 0.06,
                    edgecolor=ob_color, linewidth=0.8,
                    linestyle='-' if confirmed else '--', zorder=1
                ))
                ax.text(1, ob_top, f" OB {'✓' if confirmed else '⚠'}",
                        color=ob_color, fontsize=8, va='bottom', fontweight='bold')
        except:
            pass

        # FVG
        try:
            fvg_top    = float(data.get('fvg_top') or 0)
            fvg_bottom = float(data.get('fvg_bottom') or 0)
            if fvg_top > 0 and fvg_bottom > 0 and fvg_top != fvg_bottom:
                confirmed = data.get('fvg_confirmed', True)
                ax.add_patch(patches.Rectangle(
                    (0, fvg_bottom), n, fvg_top - fvg_bottom,
                    facecolor='#3498db', alpha=0.15 if confirmed else 0.06,
                    edgecolor='#3498db', linewidth=0.8,
                    linestyle='-' if confirmed else '--', zorder=1
                ))
                ax.text(1, fvg_top, f" FVG {'✓' if confirmed else '⚠'}",
                        color='#3498db', fontsize=8, va='bottom', fontweight='bold')
        except:
            pass
        ...
        drawn_prices = []
        all_level_prices = []
        for key in level_cfg:
            try:
                raw = levels.get(key, 0)
                p = float(str(raw).split('-')[0].strip()) if raw else 0
                if p > 0:
                    all_level_prices.append(p)
            except:
                pass
        p_range = (max(all_level_prices) - min(all_level_prices)) if len(all_level_prices) >= 2 else 1

        for key, (color, lstyle, lwidth, lbl) in level_cfg.items():
            try:
                raw = levels.get(key, 0)
                price = float(str(raw).split('-')[0].strip()) if raw else 0
                if price <= 0:
                    continue
                y_label = price
                for yp in drawn_prices:
                    if abs(y_label - yp) < p_range * 0.015:
                        y_label += p_range * 0.018
                drawn_prices.append(y_label)
                ax.axhline(y=price, color=color, linestyle=lstyle,
                           linewidth=lwidth, alpha=0.85, zorder=4)
                ax.text(n + 0.3, y_label, f"{lbl}: {price:,.4f}",
        ...
        try:
            ...
            for ann in annotations:
                price = float(ann.get('price') or 0)
                if price <= 0:
                    continue
        ...
        if 'Volume' in df_plot.columns:
            for i in range(len(df_plot)):
                try:
                    vol = float(df_plot['Volume'].iloc[i] or 0)
                    if np.isnan(vol) or vol <= 0:
                        continue
                    vc = '#26a69a' if float(df_plot['Close'].iloc[i]) >= float(df_plot['Open'].iloc[i]) else '#ef5350'
        ...
        ax_vol.set_xlim(-1, n + 16)

        legend_items = [
            patches.Patch(facecolor='#26a69a', alpha=0.8, label='Bullish'),
            patches.Patch(facecolor='#ef5350', alpha=0.8, label='Bearish'),
        ...
        plt.close(fig)
        print(f"    Chart OK — {title}")
        return result

    except Exception as e:
        print(f"    Chart failed for {title}: {e}")
        plt.close('all')
        return None
...
def build_html(data, pair, zone_level, zone_label, current_price,
    ...
    try:
        ...
        lvls = {
            "TP2": float(data["tp2"]), "TP1": float(data["tp1"]),
            "Entry": ep, "Current": float(current_price),
            "Zone": float(zone_level), "SL": float(data["sl"])
        }
    ...
    chart1_html = f'<h3 style="color:#1a1a2e;font-size:13px;margin:20px 0 6px;">📈 {tf1_label} CHART</h3><img src="data:image/png;base64,{chart1_b64}" style="width:100%;border-radius:8px;margin-bottom:4px;" />' if chart1_b64 else f'<p style="color:#aaa;font-size:12px;">📈 {tf1_label} chart unavailable this alert.</p>'
    chart2_html = f'<h3 style="color:#1a1a2e;font-size:13px;margin:16px 0 6px;">📈 {tf2_label} CHART</h3><img src="data:image/png;base64,{chart2_b64}" style="width:100%;border-radius:8px;" />' if chart2_b64 else f'<p style="color:#aaa;font-size:12px;">📈 {tf2_label} chart unavailable this alert.</p>'
...
<div style="padding:20px 24px;">
  ...
  {chart2_html}
  {price_map}
  <h3 style="color:#1a1a2e;font-size:13px;margin:20px 0 8px;">🎯 TRADE</h3>
...
def log_alert(pair, zone_level, zone_label, current_price, data):
    ...
    print(f"    Logged alert for {pair}. Total log entries: {len(alert_log)}")
...
for pair_conf in config["pairs"]:
    ...
    zones, current_price, df1, df2, tf1_label, tf2_label = detect_zones_and_candles(
        symbol, min_touches, style
    )
    ...
    for zone_level, touches in zones:
        ...
        print(f"    ZONE HIT: {name} {zone_label} at {zone_level:.5f} (dist: {dist_pct:.2f}%)")
        ...
        alerts_fired += 1
        print(f"    ✅ Done: {name} [{score}/10]")

# Always save log at end, even if no alerts fired
save_alert_log()
