...
def log_alert(pair, zone_level, zone_label, current_price, data, alert_type="zone", geo_flag=False):
    ...
    alert_log.append({
        ...
        "entry_model":        data.get("entry_model","") if data else "",
        "entry_ready_now":    data.get("entry_ready_now", False) if data else False,
        "trigger_status":     data.get("trigger_status","not_ready") if data else "not_ready",
        "trigger_tf":         data.get("trigger_tf","") if data else "",
        "trigger_kind":       data.get("trigger_kind","") if data else "",
        "trigger_level":      data.get("trigger_level",0) if data else 0,
        "trigger":            data.get("trigger","") if data else "",
        "invalidate_above":   data.get("invalidate_above", None) if data else None,
        "invalidate_below":   data.get("invalidate_below", None) if data else None,
