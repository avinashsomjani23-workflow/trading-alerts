...
def test_levels_dual_entry():
    ...
    # R-distance = raw entry -> stop. The stop carries the ONE spread (distal -
    # spread), so R = OB_width + 1*spread (proximal) / OB_width/2 + 1*spread (50pct).
    # entry no longer moves, so the whole spread cost is the stop buffer alone.
    ...
    ob_w = ob["high"] - ob["low"]
    spread = pair_conf["spread_pips"] * 0.0001
    r_mid_expected = ob_w / 2.0 + 1.0 * spread
    r_prox_expected = ob_w + 1.0 * spread
    ok &= check(abs(r_mid - r_mid_expected) < 1e-6,
                f"50pct R == OB_width/2 + 1*spread ({r_mid:.5f} vs {r_mid_expected:.5f})")
    ok &= check(abs(r_prox - r_prox_expected) < 1e-6,
                f"proximal R == OB_width + 1*spread ({r_prox:.5f} vs {r_prox_expected:.5f})")
