...
| Sweep v2 (liquidity_sweep.py, built 2026-07-18 — SWEEP_V2_DESIGN.md; tier SW + score rewire 2026-07-20) | `sweep2_present`, `sweep2_tier`, `sweep2_pierce_atr`, `sweep2_rejection_ratio`, `sweep2_follow_atr`, `sweep2_rn_aligned`, `sweep2_age_at_alert_h1`, `sweep2_pools_swept` | Stage 1 screen candidates — the rebuilt pool-anchored sweep (real-pool raid + rejection + follow-through + RN alignment). **2026-07-20: tier SW folded in** (a bare lb-3+1.5-ATR swing resting at leg start, ranked last — the normal-swing fuel read; `sweep2_tier` gains value 'SW'). **SCORE REWIRE 2026-07-20 (owner Option 1): the sweep SCORE LEG now reads this merged snapshot (liquidity_sweep.score_inputs) instead of the legacy sweep_observed — a live-behaviour change; the setup badge / OB2-rank / legacy `sweep_present` column still read legacy.** **OB-BUILD-anchored, formation-frozen snapshot** (all fields known before the alert) → eligible for the alert-time EV model. Screened on the non-null subpopulation (None on legacy zones / unevaluable). F-rule per-bucket curves mandatory; A5's exclusion applies to the LEGACY columns only. No email verdict beyond the observe-only narration; the score-leg shift is the ONLY sanctioned live change until the columns validate |
| Sweep v2 support | `sweep2_level`, `sweep2_rn_dist_atr`, `sweep2_eq_size`, `sweep2_tiers_checked` | audit / derivation support (raided price, signed RN distance, shelf size, tier-provability honesty label); NOT screened (kept out of the §5.1 manifest) |
