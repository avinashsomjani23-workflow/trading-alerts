...
| Structure signals (STRUCTURE_SIGNALS_SPEC) | `flip_pending_at_alert`, `flip_pending_dir_at_alert`, `leg_retrace_pct_at_alert`, `dr_ceiling_broken_at_ob`, `dr_floor_broken_at_ob` | Stage 1 screen candidates (added to §5.1 manifest, edge_engine.py). No email/gate change until Stage 1 validates (SIGNAL_CANDIDATES rule). (`structure_ranging_at_alert` REMOVED 2026-07-31 — the ranging flag was ripped out; see SIGNAL_CANDIDATES §4.) |
...
| PD/PW liquidity pools (DAILY_BIAS_V4_SPEC §1.3, built 2026-07-13) | `day_state_at_alert`, `pdh_status_at_alert`, `pdl_status_at_alert`, `pwh_status_at_alert`, `pwl_status_at_alert`, `dist_next_pool_above_atr`, `dist_next_pool_below_atr`, `next_pool_above_tier`, `next_pool_below_tier`, `trade_toward_pool` | Stage 1 screen candidates, judged under the spec's three pre-registered hypotheses (SWEPT_* counter-trades / toward-unspent-weekly-pool / BOTH_SIDES underperformance) at F-rule thresholds. **FILL-anchored** (derived from bars strictly before `fill_ts`, owner call 2026-07-16 — the `_at_alert` suffix is historical): they screen in Stage 1 but route to the ORDER-RULE track, never the alert-time EV model (look-ahead wall). Screened on each column's non-null subpopulation. Kill rule: none survive validation → the daily layer dies in full, columns stay. No email/gate/score change until validated |
| PD/PW pool support | `last_sweep_age_h1`, `last_sweep_tier` | descriptive context for the sweep-recency read; NOT screened (kept out of the §5.1 manifest — support, not a standalone candidate) |
...
| Decreed out | LEGACY `sweep_pts`/`sweep_present` (superseded by sweep2_*; A5 — kept only for score parity), `news_event_*` (confounder only), NAS100 | §5.1 |
...
| Regime (built 2026-07-15, replaces `efficiency_ratio_at_alert` which never reached a canonical run) | `chop_at_alert` | Stage 1 screen candidate — the ranging/trending-day regime read (the "choppy day = worse trades" hypothesis, unproven). Alert-time, closed bars only, look-ahead clean. F-rule thresholds; no email/gate/score change until validated |
| Weekly PD zone (weekly_pd.py, built 2026-07-15) | `weekly_pd_position_at_alert`, `weekly_pd_zone_at_alert`, `pd_zone_agreement_at_alert` | Stage 1 screen candidates — higher-timeframe premium/discount + H4-vs-weekly agreement. **ALERT-anchored** (bars strictly before `alert_ts`, unlike the fill-anchored pool/EQ layers) → eligible for the alert-time EV model. Screened on the non-null subpopulation (None until a completed prior week exists). Last COMPLETED week only. F-rule thresholds; no email verdict beyond the existing observe-only P2 line until validated |
| Trend-PD confluence (2026-07-08) | `trend_pd_agree` | Stage 1 categorical screen candidate — was logged but un-screened until 2026-07-16. Enters via this column, NOT via `h1_trend` (redundant with `trend_alignment`). Alert-time |
| Weekly PD support | `weekly_range_high_at_alert`, `weekly_range_low_at_alert` | raw PWH/PWL levels behind the position % — audit/derivation support; NOT screened (kept out of the §5.1 manifest) |
| EQ liquidity clusters (eq_pools.py, built 2026-07-14) | `eqh_above_dist_atr`, `eqh_above_size`, `eql_below_dist_atr`, `eql_below_size`, `eq_trade_toward`, `eq_sl_gap_atr`, `eq_sl_at_risk`, `eq_intact_above_count`, `eq_intact_below_count` | Stage 1 screen candidates under the pre-registered EQ reads (`eq_sl_at_risk` = the instant-death/SL-in-a-pool test; `eq_trade_toward` = draw-on-liquidity). **FILL-anchored** (same rationale as PD/PW pools — bars strictly before `fill_ts`): ORDER-RULE track only, non-null subpopulation. F-rule thresholds; no email/gate/score change until validated |
| EQ cluster support | `eq_last_sweep_age_h1`, `eq_last_sweep_side` | descriptive context for the cluster reads; NOT screened (kept out of the §5.1 manifest — support, not standalone candidates) |
| TP-placement audit (2026-07-15 zone-edge TP change) | `tp1_wick`, `tp1_wick_rr`, `tp1_zone_source`, `tp2_wick`, `tp2_zone_source` | trade-geometry audit twins of `tp1/tp2/tp1_rr` — they separate the "nearer TP (zone vs wick)" effect from the "lower floor (0.5R)" effect in the run's outcomes. NOT features (same rule as Trade geometry above); diagnostics only |
| Sweep v2 (liquidity_sweep.py, 2026-07-18; WS2 four-block reshape 2026-07-25) | Roll-ups `sweep2_present`, `sweep2_pools_swept`; SW block `sweep2_sw_present/_pierce_atr/_rejection_ratio/_follow_atr/_rn_aligned`; EQ block `sweep2_eq_present/_pierce_atr/_rejection_ratio/_follow_atr/_rn_aligned`; PW block `sweep2_pw_present/_pierce_atr/_rejection_ratio/_follow_atr/_rn_aligned`; PD block `sweep2_pd_present/_pierce_atr/_rejection_ratio/_follow_atr/_rn_aligned` | Stage 1 screen candidates — FOUR always-on per-tier sweep blocks (each: was this tier's pool raided in the OB's fuel leg + rejection + follow-through + RN alignment). No winner-collapse: a raid hidden behind a higher tier still shows in its own block. **ANCHORING SPLIT (WS2): SW + EQ blocks are OB-BUILD FROZEN → eligible for the alert-time EV model. PW + PD blocks are FILL-ANCHORED** (yesterday's/last-week's H/L roll; judged at the fill candle on the OB's frozen fuel window, closed bars strictly before fill_ts) → **ORDER-RULE track only** (same class as the fill-anchored pool/EQ/approach columns; banned from the §5.1 alert-time screen, look-ahead wall). Screened on the non-null subpopulation. F-rule per-bucket curves mandatory. Observe-only, A5 — gates/scores NOTHING (the legacy score leg is unchanged; the CSV winner was dropped, not rewired). |
| Sweep v2 — DECREED OUT (WS2 2026-07-25) | ~~`sweep2_tier`, `sweep2_level`, `sweep2_pierce_atr`, `sweep2_rejection_ratio`, `sweep2_follow_atr`, `sweep2_rn_aligned`, `sweep2_rn_dist_atr`~~ | REMOVED from trades.csv — redundant winner cols, reconstructable from the four per-tier blocks (winner = highest-ranked present tier). The winner still lives on the frozen `ob['sweep_v2']` snapshot for the LIVE email/score read; it is simply no longer a CSV column. Not screened (no longer exists). |
| Sweep v2 support | `sweep2_eq_size`, `sweep2_age_at_fill_h1`, `sweep2_tiers_checked` | audit / derivation support (earliest-swept EQ shelf size; fill-anchored age of the earliest raid; tier-provability honesty label incl. the fill pw/pd verdict); NOT screened (kept out of the §5.1 manifest) |
| Setup liquidity (setup_liq.py, 2026-07-20 — SWING_SWEEP_SPEC, owner-corrected) | `setup_liq_stop_present`, `setup_liq_stop_offset_atr`, `setup_liq_stop_tier`, `setup_liq_tp_present`, `setup_liq_tp_offset_atr`, `setup_liq_legextreme_swept` | Stage 1 screen candidates — this trade's OWN stop-side liquidity (Read 1: active swing in SL±0.5ATR, signed offset = survive-the-hunt vs stopped-then-runs), tp-side magnet (Read 2: TP1 on unspent liquidity = Draw on Liquidity), and leg-extreme-was-a-sweep (Read 3.2: the leg ended on a stop-run). Reads 1&2 are **LEVEL-CALC-anchored** (born from compute_phase2_levels SL/TP1, not OB-build-frozen — deterministic on the closed-bar frame, look-ahead clean) → eligible for the alert-time EV model. Read 3.2 is **OB-build-frozen** (leg geometry). Screened on the non-null subpopulation. F-rule per-bucket curves mandatory (FX/GOLD separate); no email verdict beyond the observe-only P2 narration, no gate/score change until validated |
