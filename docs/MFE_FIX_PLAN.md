...
> **RE-DERIVED 2026-08-11** on the structure-fix re-run (3,322→3,041). Generated by the autopsy scripts
> (population = resolved `tp|sl`). The taxonomy below is now the **CLEAN** version — the `mfe_intrade_r`
> fix has SHIPPED, so it replaces the old contaminated-`mfe_r` PROVISIONAL numbers. (Old run in parens.)

**Baseline tiers**
| tier | N | W | L | WR | meanR | sumR |
|---|---|---|---|---|---|---|
| all resolved (no news filter) | 1,990 | 589 | 1,401 | 29.6% | −0.112 | −223 |
| LOOSE `news_fill==0` (frozen baseline) | 1,370 | 409 | 961 | 29.9% | −0.104 | −143 |
| STRICT (also `news_open==0`, autopsy) | 1,162 | 320 | 842 | 27.5% | −0.174 | −202 |
| news-open segment removed by strict | 208 | 89/119 | — | 42.8% | +0.284 | +59 |

**Strict clean losers (N=842) — CLEAN (trustworthy):**
- died in ≤1 bar: **485 (57.6%)**
- stop bar was a sweep (`sl_bar_was_sweep`): 448 (53.2%)
- swept then ran +1R: 97 (11.5%) · swept then ran +2R: 67 (8.0%)
- MFE peak AFTER stop (the `mfe_r` contamination the fix removed): 547 (65.0%)

**Strict losers — CLEAN taxonomy (`mfe_intrade_r`-based, fix shipped; sums to 842):**
- instant death (mfe0 & ≤1 bar): **485** · slow bleed (mfe0 & >1 bar): 39
- weak reaction (0–1R, peak pre-stop): 197 · real give-back (≥1R, peak pre-stop): 121
- NOTE: instant-death jumped 138→485 vs the OLD PROVISIONAL count — that gap was the whole point of
  the fix: the contaminated `mfe_r` had mislabelled ~350 dead-on-arrival losers as "had profit."
