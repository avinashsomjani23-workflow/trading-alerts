...
| direction | :1476 | OB build (immutable) | verified | detection audit 3e: immutable event fact |
| bias | :1477 | derived from direction | verified | deterministic at :1477 (LONG if bullish else SHORT) |
| model | :1478 | constant | verified | literal "h1_only" |
| event | :1479 | OB build | verified | deterministic label from immutable bos_tag/bos_tier |
...
| ob_to_fill_hours | :1527 (calc :1068-1075) | fill | verified | deterministic from immutable ob_timestamp + fill_ts; None if never filled |
...
| pd_alignment | :1541 (calc :241) | row build | verified | deterministic from bias + pd_zone |
...
| reversed_from_extreme | :1547 (calc :1257) | row build, derived | verified | deterministic; 0.0 ambiguity documented at :1077-1081 |
...
| session | :1555 (calc :130) | alert | verified | fixed UTC-hour buckets. CAVEAT: UTC-fixed, NOT DST-aware — session edges shift 1h half the year (killzone columns ARE DST-aware; known asymmetry) |
...
| bos_tag / bos_tier | :1562-1563 | OB build (immutable) | verified | detection audit 3e: immutable event facts |
...
| atr_at_ob | :1584 (calc :1305) | OB build (frozen by design) | verified | formation ATR, frozen BY DESIGN (detection audit 3e) |
...
| ob_session | :1613 (calc :123) | OB build ts | verified | deterministic from immutable ob_timestamp; UTC-fixed caveat as session |
| fill_session | :1614 (calc :130) | fill | verified | CAVEAT: never_filled rows fall back to ALERT hour (:118-120) — population mixes fill-time and alert-time labels; never_filled rows are audit-only |
