...
backtest/test_h1_only.py::test_edge_lab_columns (drives live _build_row). Areas B
(ATR normalisation) + D (insight population) still pending.
...
| bos_timestamp | h1_only_simulator.py `_build_row` (`ob.get("bos_timestamp")`) | OB build (backtest: frozen; LIVE: re-stamped on refresh) | verified (backtest) / DISCUSSION POINT (live) | FREEZE (Deep Value A, 2026-07-10): FROZEN in the canonical CSV (replay merges by exact ob_timestamp, refreshes only fvg — guards test_ob_alert_freeze.py bite-proven). **LIVE DIVERGENCE:** zone.py refresh RE-STAMPS bos_timestamp (:271) on a proximity-fallback match (find_matching_slate_zone branch b, smc_radar.py:3328 — same price, different break), so a refreshed live zone carries the LATEST break's ts, not formation's. Ledger's "immutable" was true for backtest, NOT live. Locked by test_zone_roundtrip.py::test_ob_identity_fields_freeze_split_on_refresh (bite-proven). Not fixed — flagged as a DISCUSSION POINT (is re-stamping to the newer confirming break intended, or a leak? P&L-neutral for canonical CSV). 2026-07-02 detection audit 3e classification; alert>bos guard replay_engine.py:529-554. 2026-07-09 EVENT-CANDLE FIX: derives from the TRUE break candle, not the confirmation candle. **2026-07-10 GATES REMOVED**: with the distance buffer now 0, the true-break candle == the fire candle on every continuous break (differs only across a weekend gap), so bos_timestamp = the bar the close first crossed the level. bos_idx=_idx_from_ts(candle_ts) (smc_radar.py:866) inherits it. See event_candle_delta row |
...
| sl_distance_atr | h1_only_simulator.py (calc in _build_row beside atr_at_ob) | fill | verified | 2026-07-08; abs(entry - sl_initial)/ob['h1_atr']; risk width in ATR; uses sl_initial (traded stop) NOT sl_raw; point-in-time clean; None if no ATR; ENCODED (replaces ex-pasted corrupted CSV col); guard test_h1_only.py::test_edge_lab_columns. Area C 2026-07-10: population reachability-verified — 28,191/33,838 bit-exact, 5,647 reachable within the 6dp-atr round-band, 0 unreachable (divides full-precision _h1_atr while atr_at_ob stores 6dp; precision-limited recompute, formula correct). Guard test_truth_ledger.py::test_area_c_sl_distance_atr_reachable_via_atr_rounding_band (bite-proven) |
...
| bos_tag / bos_tier | :1562-1563 | OB build (bos_tag: frozen; bos_tier: backtest frozen / LIVE re-stamped) | verified (backtest) / DISCUSSION POINT (bos_tier live) | FREEZE (Deep Value A, 2026-07-10): bos_tag frozen BOTH paths. bos_tier FROZEN in canonical CSV (replay refreshes only fvg) but **LIVE zone.py refresh RE-STAMPS bos_tier (:268)** on a proximity-fallback match — same divergence class as bos_timestamp. Guards test_ob_alert_freeze.py::test_ob_build_fields_stamped_once_in_radar + ::test_replay_merge_only_refreshes_fvg_not_identity_fields + test_zone_roundtrip.py::test_ob_identity_fields_freeze_split_on_refresh (all bite-proven RED). detection audit 3e: immutable event facts (holds for backtest + for bos_tag live) |
...
| ob_range_atr | :1581 (calc :1275) | row build from frozen high/low | verified | deterministic from immutable high/low + frozen h1_atr |
| fvg_size_atr | :1582 (calc :1287) | row build from refreshed fvg | **fixed** | Fix 3c LANDED (fvg refresh on re-surface, replay_engine.py:424); alert-time snapshot guard PASS |
| impulse_leg_atr | :1583 (calc :1299) | row build from frozen prices | verified | deterministic from immutable impulse_start_price + bos_swing_price |
| atr_at_ob | :1584 (calc :1305) | OB build (backtest: frozen; LIVE: re-stamped) | verified (backtest) / DISCUSSION POINT (live) | FREEZE (Deep Value A, 2026-07-10): reads ob['h1_atr'] (formation ATR), NOT the per-scan alert['h1_atr'] — that distinction IS the freeze, asserted by test_ob_alert_freeze.py::test_ob_build_fields_read_directly_no_snapshot (bite-proven). FROZEN in canonical CSV (replay merge refreshes only fvg). **LIVE DIVERGENCE:** zone.py refresh RE-STAMPS h1_atr (:261) on a proximity-fallback match, so a refreshed live zone's atr_at_ob is the refresh-scan ATR, not formation ATR — this silently warps every LIVE *_atr normalisation (Area B) for refreshed zones. Canonical CSV unaffected. Locked by test_zone_roundtrip.py::test_ob_identity_fields_freeze_split_on_refresh (bite-proven). DISCUSSION POINT, not fixed. Was "frozen BY DESIGN (detection audit 3e)" — true for backtest only |
...
| eligible_for_headline / headline_exclusion | h1_only_reporting.py:1230-1231 | CSV write, from THE one rule _headline_exclusion (:142-176) | verified | Area C recompute-verified (0 mismatch, 33,838 rows) from exit_reason + ist_blocked + weekend_blocked. self-describing headline membership; sum(pnl_usd where eligible) == headline, enforced by reconcile assert :3435-3449 (Area D will re-assert that reconcile ON the file) |
