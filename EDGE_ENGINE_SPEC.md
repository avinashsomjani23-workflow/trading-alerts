...
| Weekly PD zone (weekly_pd.py, built 2026-07-15) | `weekly_pd_position_at_alert`, `weekly_pd_zone_at_alert`, `pd_zone_agreement_at_alert` | Stage 1 screen candidates — higher-timeframe premium/discount + H4-vs-weekly agreement. Alert-time, last COMPLETED week only. F-rule thresholds; no email verdict beyond the existing observe-only P2 line until validated |
...
| EQ liquidity clusters (eq_pools.py, built 2026-07-14) | `eqh_above_dist_atr`, `eqh_above_size`, `eql_below_dist_atr`, `eql_below_size`, `eq_trade_toward`, `eq_sl_gap_atr`, `eq_sl_at_risk` | Stage 1 screen candidates under the pre-registered EQ reads (`eq_sl_at_risk` = the instant-death/SL-in-a-pool test; `eq_trade_toward` = draw-on-liquidity). F-rule thresholds; no email/gate/score change until validated |
| EQ cluster support | `eq_last_sweep_age_h1`, `eq_last_sweep_side`, `eq_intact_above_count`, `eq_intact_below_count` | descriptive context for the cluster reads; screened only if an EQ hypothesis survives (support, not standalone candidates) |
