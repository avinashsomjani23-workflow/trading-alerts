...
def build_weekly_analysis(weekly_alerts, wins, losses, pending, win_rate, pair_stats):
    ...
    alert_data_json    = json.dumps(alert_data, indent=2)
    session_stats_json = json.dumps(session_stats, indent=2)
    pair_stats_json    = json.dumps(pair_stats, indent=2)
    loss_clusters_str  = str(loss_clusters) if loss_clusters else "None identified"
...
{{
  ...
  "confidence_calibration": "one sentence — are high scored alerts winning more",
  "session_summary": "which session had best and worst win rate with numbers",
  ...
  "scorecard_rows": [
    {{
      ...
      "comment": "one sharp observation on this trade"
    }}
  ]
}}"""
...
def insight_card(title, color, content):
    return (f'<div style="background:#f8f9fa;padding:13px 15px;border-radius:10px;margin-bottom:12px;border-left:4px solid {color};">'
            f'<p style="font-size:10px;color:{color};margin:0 0 4px;text-transform:uppercase;font-weight:bold;letter-spacing:0.5px;">{title}</p>'
...
def build_weekly_html(data, weekly_alerts, wins, losses, pending, win_rate):
    ...
    for row in data.get("scorecard_rows",[]):
        ...
        scorecard_rows += f"""<tr style="background:{bg};border-bottom:1px solid #eee;">
          ...
          <td style="padding:7px 10px;font-size:12px;font-weight:bold;">{outcome_sym}</td>
...
<div style="display:flex;background:#f8f9fa;border-bottom:1px solid #eee;">
  ...
  <div style="flex:1;padding:14px 16px;text-align:center;border-right:1px solid #eee;">
    ...
    <p style="margin:4px 0 0;font-size:24px;font-weight:bold;color:#1a1a2e;">{total_alerts}</p>
  </div>
  <div style="flex:1;padding:14px 16px;text-align:center;">
    ...
    <p style="margin:4px 0 0;font-size:18px;font-weight:bold;">
      <span style="color:#27ae60;">{wins}</span> / <span style="color:#e74c3c;">{losses}</span> / <span style="color:#999;">{pending}</span>
    </p>
  </div>
</div>
...
<div style="display:flex;background:#f8f9fa;border-bottom:1px solid #eee;">
  ...
  <div style="flex:1;padding:12px 20px;text-align:center;">
    <p style="margin:0;font-size:10px;color:#888;">Weakest Pair</p>
